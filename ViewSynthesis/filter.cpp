...
void GaussianFilter_8u_ignore_boudary(const Mat& src, Mat& dest, int r, float sigma, const Mat& mask)
{
    ...
    if(src.channels()==3)
    {
        ...
        {
            uchar* d = dest.ptr<uchar>(j);
            const uchar* msk = mask.ptr<uchar>(j);
            for(int i=r;i<src.cols-r;i++)
            {
                if(msk[i]!=0)
                {
...
void GaussianFilter(const Mat src, Mat& dest, int r, float sigma, int method, const Mat& mask)
{
    if(mask.empty())
    {
